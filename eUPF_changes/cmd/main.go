...
func main() {
	...
	for _, ifaceName := range config.Conf.InterfaceName {
		...
		// logger.XdpLog.Infof("Attached XDP program to iface: %q (index %d)", iface.Name, iface.Index)
		color := "\033[1;32m" // Blue color (change to the desired color code)
		reset := "\033[0m"    // Reset color
		currentTime := time.Now().UTC().Format("2006-01-02T15:04:05Z")
		fmt.Printf("%s %s[INFO][HEXA_UPF][XDP] %sAttached XDP program to iface: %q (index %d)\n", currentTime, color, reset, iface.Name, iface.Index)

	}
	// log.Info().Msgf("Initialize resources: UEIP pool (CIDR: \"%s\"), TEID pool (size: %d)", config.Conf.UEIPPool, config.Conf.FTEIDPool)
	logger.AppLog.Infof("Initialize resources")
	var err error
	...
	for {
		...
		case <-stopper:
			...
			logger.Pfcplog.Infof("Received signal, exiting program...")
