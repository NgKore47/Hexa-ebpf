...
static __always_inline enum xdp_action route_ipv4(struct xdp_md *ctx, struct ethhdr *eth, const struct iphdr *ip4) {
    ...
    if (!statistic) {
        upf_printk("boom passed your packet ");
        return XDP_PASS;
    }
    upf_printk("in route ipv4");
    ...
    upf_printk("source addr :%d destination :%d ",&ip4->saddr , &ip4->daddr);
    int rc = bpf_fib_lookup(ctx, &fib_params, sizeof(fib_params), 0 /*BPF_FIB_LOOKUP_OUTPUT*/);
    upf_printk("from fib params source addr :%d destination :%d ",&fib_params.ipv4_src , &fib_params.ipv4_dst);

    upf_printk("starting switch case rc is %d", rc);
    switch (rc) {
